...
function renderAmounts(
  amounts: Amounts = {
    amount_coin: new BigNumber(-1),
    amount_usd: new BigNumber(-1),
  },
) {
  if (amounts.amount_coin.eq(-1)) {
    ...
    return (
      <div
        className={
          'amounts' + (amounts.amount_coin.isZero() ? ' amounts-empty' : '')
        }
      >
    ...
    case ChainKey.POL:
      Modal.info({
        ...
        content: (
          <div>
            <p>
              You can get some free MATIC using those faucets. It should be
              enough to exchange or move your funds.
            </p>
            <ul>
              <li>
                <a
    ...
    case ChainKey.DAI:
      Modal.info({
        ...
        content: (
          <div>
            <p>
              You can get some free DAI using those faucets. It should be enough
              to exchange or move your funds.
            </p>
            <ul>
              <li>
                <a
      ...
      Modal.info({
        ...
        content: (
          <div>
            ...
            <a
              href="https://www.okex.com/okexchain"
              target="_blank"
              rel="nofollow noreferrer"
            >
...
for (const chainId of visibleChains) {
  ...
  tooltipsEmpty[chain.key] = (
    <>
      <span>
        The ${chain.name} requires ${chain.coin} to pay for gas. Without it you
        won't be able to do anything on this chain.
      </span>
...
const calculateWalletSummary = (wallet: Wallet, totalSumUsd: BigNumber) => {
  ...
  visibleChains.forEach((chainId) => {
    ...
    wallet.portfolio[chain.key]?.forEach((tokenAmount) => {
      summary.chains[chain.key].amount_usd = new BigNumber(
        summary.chains[chain.key].amount_usd,
      ); // chainKeysToObject retruns string

      summary.chains[chain.key].amount_usd = tokenAmount.priceUSD
        ? summary.chains[chain.key].amount_usd.plus(
            new BigNumber(tokenAmount.amount).times(
              new BigNumber(tokenAmount.priceUSD),
            ),
          )
        : summary.chains[chain.key].amount_usd;
    });
    summary.chains[chain.key].percentage_of_portfolio = wallet.portfolio[
      chain.key
    ]
      ?.reduce(
        (sum, current) =>
          current.priceUSD
            ? sum.plus(
                new BigNumber(current.amount).times(
                  new BigNumber(current.priceUSD),
                ),
              )
            : sum,
        new BigNumber(0),
...
const Dashboard = ({ account }: any) => {
  ...
  const buildRows = () => {
      ...
      registeredWallets.forEach((wallet) => {
        for (const chainId of visibleChains) {
          ...
          if (cellContent.amount_coin.gt(0)) {
            ...
            coinRow.portfolio.amount_usd = coinRow.portfolio.amount_usd.plus(
              cellContent.amount_usd,
            );
          }
        } // for each chain
      }); // for each wallet
    ...
    generatedRows.sort((a, b) =>
      b.portfolio.amount_coin.minus(a.portfolio.amount_coin).toNumber(),
    ); // DESC token
    generatedRows.sort((a, b) =>
      b.portfolio.amount_usd.minus(a.portfolio.amount_usd).toNumber(),
    ); // DESC usd
    ...
    for (const chainId of visibleChains) {
      ...
      chainPortfolio.forEach((token) => {
        const exists = coins.find(
          (existingCoin) =>
            existingCoin.chains[chain.id]?.address === token.address,
        );
        if (!exists) {
          let symbolExists = coins.find(
            (existingCoin) => existingCoin.key === token.symbol,
          );
          ...
          if (symbolExists) {
            ...
            while (symbolExists) {
              ...
              symbolExists = coins.find(
                // eslint-disable-next-line no-loop-func
                (existingCoin) =>
                  existingCoin.key === token.symbol + '_' + symbol_id,
              );
            }
            symbol += '_' + symbol_id;
          }
    ...
    if (isSubscribed) {
      setRegisteredWallets(
        registeredWallets.map((old) =>
          old.address === wallet.address ? wallet : old,
        ),
      );
  ...
  const getModalAddressSuggestion = () => {
    ...
    const web3Account =
      account.address && addedWallets.indexOf(account.address) === -1
        ? account.address
        : '';
  ...
  return (
    <Content className="site-layout dashboard">
      ...
      >
        <div
          className="connected-wallets-section"
