...
import { LineChartV3Skeleton } from './LineChartV3Skeleton';
...
export interface LineChartV3Props<V, T extends ChartDataPoint<V>> {
  data: T[];
  theme: {
    lineColor?: string;
    areaTopColor?: string;
...
export const LineChartV3 = <V, T extends ChartDataPoint<V>>({
  data,
  theme,
  dateFormat,
  dataSetId,
  ...
  enableTooltip = true,
  isLoading,
}: LineChartV3Props<V, T>) => {
  const muiTheme = useTheme();
  const chartContainerRef = useRef<HTMLDivElement>(null);

  if (isLoading) {
    return <LineChartV3Skeleton />;
  }
