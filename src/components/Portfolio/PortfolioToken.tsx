...
import { AccordionDetails, Avatar, Badge, ButtonBase, Grid, styled, Tooltip } from '@mui/material';
...
function PortfolioToken({ token }: PortfolioTokenProps) {
  ...
  return (
    <WalletCardContainer sx={{ padding: '0!important' }}>
      <CustomAccordion expanded={isExpanded} key={generateKey(token.symbol)} disableGutters onChange={handleChange}>
        <AccordionSummary
          sx={{
            padding: 0,
          ...
          <Grid container display="flex" alignItems="center">
            <Grid item xs={2}>
              {hasMultipleChains ? <Avatar>
                ...
                <Badge
                  ...
                  badgeContent={
                    <SmallAvatar alt={token.chains[0].name} src={token.chains[0].logoURI} sx={{ width: 15, height: 15 }} />
                  }
                >
            ...
            <Grid item xs={5}>
              ...
              {!hasMultipleChains ? <TypographySecondary>{token.chains[0].name}</TypographySecondary> :
              isExpanded ?
                <TypographySecondary>{t('navbar.walletMenu.numberOfChains', { numberOfChains: token.chains?.length })}</TypographySecondary> :
            ...
            <ButtonBase
              ...
              <Grid container display="flex" alignItems="center">
                ...
                <Grid item xs={5} textAlign="left">
                  <TypographyPrimary
                    sx={{ fontSize: '0.875rem', lineHeight: '1.125rem' }}>{token.symbol}</TypographyPrimary>
                  <TypographySecondary
                    sx={{ fontSize: '0.625rem', lineHeight: '0.875rem' }}>{chain.name}</TypographySecondary>
                </Grid>
                <Grid item xs={5} style={{ textAlign: 'right' }}>
                  <TypographyPrimary sx={{
                    fontSize: '0.875rem',
                    lineHeight: '1.125rem',
                  }}>{chain.formattedBalance?.toFixed(2)}</TypographyPrimary>
                  <TypographySecondary sx={{
                    fontSize: '0.625rem',
                    lineHeight: '0.875rem',
                  }}>${chain.totalPriceUSD?.toFixed(2)}</TypographySecondary>
                </Grid>
              </Grid>
            </ButtonBase>
          ))}
