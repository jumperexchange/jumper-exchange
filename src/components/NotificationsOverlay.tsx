...
function NotificationOverlay() {
  ...
  const dismissOverlay = async () =>{
    await animate(`#${NOTIFICATION_OVERLAY_CSS_ID}`, {
      right: [NOTIFICATION_OVERLAY_VERTICAL_DISPLAY_POSITION, NOTIFICATION_OVERLAY_VERTICAL_START_POSITION],
      opacity:[1,0]
    },{
      duration:0.2,
      easing: "ease-in"
  ...
  useEffect(() => {
    animate(`#${NOTIFICATION_OVERLAY_CSS_ID}`,{
  ...
  return (
    <div id={NOTIFICATION_OVERLAY_CSS_ID} style={{right: NOTIFICATION_OVERLAY_VERTICAL_START_POSITION}}>
      <div>
      ...
      <Button type="link" danger onClick={dismissOverlay}> Dismiss</Button>
      </div>
    </div>
  )
}
