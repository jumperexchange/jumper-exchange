...
const SwapXpollinate = ({
  ...
  // Advanced Options
  const [optionInfiniteApproval, setOptionInfiniteApproval] = useState<boolean>(
    true
  );
  ...
  const [syncStatus, setSyncStatus] = useState<
    Record<number, SubgraphSyncRecord>
  >();
  const [activeTransactions, setActiveTransactions] = useState<
    Array<ActiveTransaction>
  >([]);
  const [updatingActiveTransactions, setUpdatingActiveTransactions] = useState<
    boolean
  >(false);
  const [historicTransaction, setHistoricTransactions] = useState<
    Array<HistoricalTransaction>
  >([]);
  const [liquidity, setLiquidity] = useState<Array<any>>([]);
  const [updatingLiquidity, setUpdatingLiquidity] = useState<boolean>(false);
  const [updateHistoricTransactions, setUpdateHistoricTransactions] = useState<
    boolean
  >(true);
  const [
    updatingHistoricTransactions,
    setUpdatingHistoricTransactions,
  ...
  const getLiquidity = async (chains: Chain[]) => {
    ...
    const liq = await Promise.all(
      chains.map(async (chain) => {
        ...
        // parse
        return res.router?.assetBalances
          ?.map((bal: { amount: string; id: string }) => {
            const assetId = bal.id.split("-")[0].toLowerCase();
            const coin = defaultCoins.find(
              (coin) => coin.chains[chain.key]?.id === assetId
            );
            ...
            return {
              ...
              liquidity: new BigNumber(
                utils.formatUnits(bal.amount, token.decimals)
              ),
            };
          })
          .filter((x: any) => !!x);
  ...
  const updateBalances = useCallback(
    async (address: string) => {
      ...
      await getBalancesForWallet(
        address,
        transferChains.map((chain) => chain.id)
      ).then(setBalances);
  ...
  const handleActivateOption = () => {
    if (
      ...
      let activeOption: any = {
        receivingAddress: optionReceivingAddress,
              ...
              header={
                ...
                >
                  ...
                  {!sdk ? (
                    ...
                    liquidity
                      .reduce(
                        (prev: BigNumber, cur) => prev.plus(cur.liquidity),
                        new BigNumber(0)
                      )
                      .shiftedBy(-3)
                      .toFixed(0) + "k"
                  ...
                  <Collapse.Panel header={`Advanced Options`} key="1">
                    ...
                    <Row style={{ marginTop: 24 }} justify={"center"}>
                      ...
                      <Button
                        shape="round"
                        type="primary"
                        onClick={(e) => handleActivateOption()}
                      >
                        Submit
                      </Button>
                    </Row>
