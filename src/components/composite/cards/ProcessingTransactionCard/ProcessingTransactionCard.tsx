...
import { type ProcessingTransactionCardProps } from './types';
import { getStatusConfig } from './utils';
import { Timer } from './Timer';

const TokenEntityStack: FC<{ token: ExtendedToken; zIndex: number }> = ({
  token,
  zIndex,
}) => (
    ...
    <ProcessingTransactionCardContainer status={status}>
      ...
      <Stack direction="row" spacing={2} alignItems="center">
        <Stack direction="row" spacing={-1.25}>
          <TokenEntityStack token={fromToken} zIndex={1} />
          <TokenEntityStack token={toToken} zIndex={0} />
        </Stack>
