...
export const RequestWithdrawView: FC<RequestWithdrawViewProps> = ({
  ...
  const { toAmount } = useTokenFormatters();
      ...
      <RequestRedeemModalContentContainer>
        ...
        {claims.map((formattedClaim) => (
          <ProcessingTransactionCard
            ...
            onClick={() => onClaimClick(formattedClaim.id)}
            // onClick={() => formattedClaim.status === 'success' ?  onClaimClick(formattedClaim.id) : undefined}
          />
        ))}
        ...
        <RequestRedeemModalFieldsContainer>
          <TokenAmountInput
            ...
            hintEndAdornment={`/ ${toAmount(maxDepositTokenBalance)}`}
            endAdornment={
              <TokenAmountInputPercentages
                decimals={depositTokenBalance.token.decimals}
