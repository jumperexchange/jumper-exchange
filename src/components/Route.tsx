    ...
    switch (step.action.type) {
      case 'swap':
        ...
        return {
          ...
          )} for ${formatTokenAmount(swapAction.toToken, swapEstimate.toAmount)} via ${
            swapAction.tool
          }`,
        }
      case 'cross':
        ...
        return {
          title: 'Cross Chains',
          description: `${chainNames[getChainById(crossAction.chainId).key]}: ${formatTokenAmount(
            crossAction.token,
            crossEstimate.fromAmount,
          )} to ${chainNames[getChainById(crossAction.toChainId).key]}: ${formatTokenAmount(
            crossAction.toToken,
            crossEstimate.toAmount,
          )} via ${crossAction.tool}`,
        }
      case 'withdraw':
        return {
          title: 'Withdraw',
