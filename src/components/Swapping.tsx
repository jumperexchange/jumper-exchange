...
import { Chain, ChainKey, ChainPortfolio, CrossAction, CrossEstimate, CrossStep, Execution, getChainById, getChainByKey, getIcon, LiFiStep, Step, SwapAction, SwapEstimate, SwapStep, Token } from '../types'
...
const Swapping = ({ route, updateRoute, onSwapDone }: SwappingProps) => {
  ...
  // Swap
  const updateStatus = (step: Step, status: Execution) => {
  ...
  const triggerSwap = async (step: SwapStep, previousStep?: Step) => {
  ...
  const triggerCross = async (step: Step, previousStep?: Step) => {
    if (!web3.account || !web3.library) return
  ...
  const parseStepToTimeline = (step: Step, index: number, route: Array<Step>) => {
    ...
    switch (step.action.type) {
      ...
      case 'lifi': {
        return [
          <Timeline.Item position={isMobile ? 'right' : 'right'} key={index + '_left'} color={color}>
            <h4>LiFi Contract from {getChainAvatar(getChainById(step.action.chainId).key)} to {getChainAvatar(getChainById(step.action.toChainId).key)}</h4>
            <span>{formatTokenAmount(step.action.token, step.estimate?.fromAmount)} <ArrowRightOutlined /> {formatTokenAmount(step.action.toToken, step.estimate?.toAmount)}</span>
          </Timeline.Item>,
          <Timeline.Item position={isMobile ? 'right' : 'left'} key={index + '_right'} color={color} dot={isLoading ? <LoadingOutlined /> : null}>
            {executionSteps}
  ...
  const triggerStep = async (index: number, route: Array<Step>) => {
    setIsSwapping(true)
    ...
    try {
      switch (step.action.type) {
        ...
        case 'cross':
          return await triggerCross(step as CrossStep, previousStep)
        case 'lifi':
          return await triggerLifi(step as LiFiStep, previousStep)
        default:
          setIsSwapping(false)
  ...
  const triggerLifi = async (step: LiFiStep, previousStep?: Step) => {
    // ensure chain is set
    if (web3.chainId !== step.action.chainId) {
      if (!(await checkChain(step))) return
    }

    return lifinance.executeLifi(web3.library!.getSigner(), step, (status: Execution) => updateStatus(route[0], status))
  }
  ...
  const getRecevingInfo = (step: Step) => {
    ...
    switch (step.action.type) {
      ...
      case 'swap':
        ...
        break
      case 'lifi':
        toChain = getChainById((step as LiFiStep).action.toChainId)
        toToken = (step as LiFiStep).action.toToken
        break
    }
    return { toChain, toToken }
  }

  const getFinalBalace = async (route: Step[]) => {
