...
const Swapping = ({ route, updateRoute, onSwapDone }: SwappingProps) => {
  ...
  const triggerSwap = async (step: SwapStep, previousStep?: TransferStep) => {
    ...
    switch (swapAction.tool) {
      ...
      default:
        return await executeUniswap(swapAction.chainId, web3.library.getSigner(), swapAction.token, swapAction.toToken, fromAmount, fromAddress, toAddress, swapEstimate.data.routerAddress, swapEstimate.data.path, (status: Execution) => updateStatus(step, status))
    }
  }
