...
export const BerachainTooltipTokens = ({
  ...
  const apys = data?.annual_change_ratios?.map((d, index) => {
    const [chain_id, token_address] = data?.incentive_ids?.[index]?.split('-') ?? []
    return {
      // chain_id: 1,
      ...
      incentive_token_price_value: data?.incentive_token_price_values?.[index],
      incentive_token_total_supply_value: data?.incentive_token_total_supply_values?.[index],
    }
    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
      {fetchedTokens.map((token, index) => (
        ...
        >
          ...
          <Typography variant="bodyXSmallStrong">{apys[index].annual_change_ratio ? t('format.percent', { value: apys[index].annual_change_ratio }) : 'N/A'}</Typography>
        </Box>
      ))}
    </Box>
  );
