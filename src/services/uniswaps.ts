...
export const getSwapCall = async (signer: JsonRpcSigner, chainId: number, destAddress: string, srcToken: string, destToken: string, srcAmount: string, destAmount: string, routerAddress: string, path: Array<string>) => {
  const contract = new ethers.Contract(routerAddress, uniswapRouter02ABI, signer)
...
export const swap = async (signer: JsonRpcSigner, chainId: number, srcToken: string, destToken: string, destAddress: string, srcAmount: string, routerAddress: string, path: Array<string>) => {
  const data = await getSwapCall(signer, chainId, destAddress, srcToken, destToken, srcAmount, '1', routerAddress, path)
