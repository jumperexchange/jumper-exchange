    ...
    test(
      ...
      async ({ page }) => {
        ...
        await test.step('Verify route priority options', async () => {
          await clickItemInSettingsMenu(
            page,
            SETTINGS_MENU.ROUTE_PRIORITY.LABEL,
          );
          await checkItemInSettingsMenu(
            page,
          ...
          await expect(
            page.locator('span.MuiBadge-badge.MuiBadge-colorInfo'),
          ).toBeVisible();
        });
        ...
        await test.step('Verify gas price options', async () => {
          ...
          await checkItemInSettingsMenu(page, SETTINGS_MENU.GAS_PRICE.SLOW, {
            enabled: true,
          });
          await checkItemInSettingsMenu(page, SETTINGS_MENU.GAS_PRICE.FAST, {
            enabled: true,
          });
        ...
        await test.step('Verify slippage settings', async () => {
          ...
          await checkItemInSettingsMenu(page, SETTINGS_MENU.SLIPPAGE.AUTO, {
            visible: true,
          });
          ...
          await expect(
            page.getByText(SETTINGS_MENU.SLIPPAGE.WARNING_MESSAGE),
          ).toBeVisible();
          // Close max slippage settings
          await clickItemInSettingsMenu(page, SETTINGS_MENU.SLIPPAGE.LABEL);
          // Verify that the slippage value is set to the custom value
          await checkSettingInSettingsMenu(page, `${slippageValue}%`, {
            visible: true,
          });
          await expect(
            page.locator('span.MuiBadge-badge.MuiBadge-colorWarning'),
          ).toBeVisible();
        });
        ...
        await test.step('Verify Bridge Settings - Deselect and select 1 bridge', async () => {
          ...
          const firstBridgeItem = bridgeListItem.locator(
            'xpath=(//div[@role="button"])[1]',
          );
          ...
          const firstBridgeCheckbox = firstBridgeItem
            .locator('[data-testid="CheckBoxIcon"]')
            .first();
        ...
        await test.step('Reset settings', async () => {
          await checkItemInSettingsMenu(page, SETTINGS_MENU.RESET.BUTTON, {
            visible: true,
          });
          ...
          await page
            .getByRole('button', {
              name: SETTINGS_MENU.RESET.DIALOG_CONFIRM_BUTTON,
            })
            .click();
          // Verify that the reset settings button disappeared after reset
          await checkSettingInSettingsMenu(page, SETTINGS_MENU.RESET.BUTTON, {
            invisible: true,
          });
          // Verify that all values are reset after reset
          await checkSettingInSettingsMenu(
            page,
            SETTINGS_MENU.ROUTE_PRIORITY.BEST_RETURN,
            { visible: true },
          );
          ...
          await checkSettingInSettingsMenu(page, SETTINGS_MENU.SLIPPAGE.AUTO, {
            visible: true,
          });
