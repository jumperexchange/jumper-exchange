...
async function verifyItemsCount(
  page: Page,
  locator: string,
  amount: number,
): Promise<void> {
...
test.describe('Zap Morpho Katana Page', () => {
  ...
  test.skip('Should connect EVM Metamask wallet', async ({
    context,
    page,
    extensionId,
  }) => {
    ...
    await test.step(
      qase(33, 'should verify the number of Bridges is higher than 1'),
      async () => {
    ...
    await test.step(
      qase(32, 'Verify the url of Discover Morpho link'),
      async () => {
