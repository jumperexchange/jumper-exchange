...
import { checkTabsInHeader } from './testData/menuFunctions';
 ...
 async function verifyBridgesCount(page: Page, numberOfBridges: number): Promise<void> {
    const widgetContainer = page.getByTestId('zap-widget-container');
    const listOfBridges = widgetContainer.locator('ul.MuiList-root.MuiList-padding div[role="button"]');
    await expect(listOfBridges).toHaveCount(numberOfBridges);
  }

test.describe('Zap Morpho Katana Page', () => {
    test.beforeEach(async ({ page }) => {
        await page.goto('/zap/morpho-katana-gauntlet-usdc');
